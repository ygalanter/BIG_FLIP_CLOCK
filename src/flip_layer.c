...
static void layer_update_callback(Layer *me, GContext* ctx) {
	...
	//graphics_draw_rect(ctx, GRect(0,0,layer_bounds.size.w,layer_bounds.size.h));
...
void animationUpdate(struct Animation *animation, const uint32_t time_normalized){
   ...
   if(NULL != flip_layer->anim_resized_image){
...
void animation_stopped(Animation *animation, bool finished, void *data) {
  ...
  if (flip_layer->delayedDestroy) animation_destroy(flip_layer->animation);
}
...
void flip_layer_destroy(FlipLayer *flip_layer){
	...
	if(flip_layer->animation && !flip_layer->isAnimating){
